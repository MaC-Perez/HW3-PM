...
parameters <- list(logK=10, 
                   logr=log(0.4), 
                   logm=log(2),   #positive scale for m because I am estimating m now 
                   logSigmaC=log(0.05), 
                   logSigmaI=log(0.2),
                   #logq = -2, 
...
#I couldnt change the folder to run the cpp file
setwd("C:/Users/macristina.perez/Documents/UMassD/Classes/2023/Fall_2023/Population Modeling/HW/HW3/Perez/PT_model")
...
compile("HW3_Perez.cpp")
dyn.load(dynlib(dll_name))

## Make a function object / Shaefer model
...
B <- summary(sdr, type = "report") %>% 
  as.data.frame() %>% 
  janitor::clean_names() %>% 
  rownames_to_column(var="type") %>% 
  filter(str_detect(type, "biomass"))

plot_1 <- summary(sdr, type = "report") %>% 
  as.data.frame() %>% 
  janitor::clean_names() %>% 
  rownames_to_column(var="type") %>% 
  filter(str_detect(type, "biomass")) %>% 
